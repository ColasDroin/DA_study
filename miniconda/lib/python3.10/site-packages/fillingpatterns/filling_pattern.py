...
class FillingPatternSingleBeam(object):
    def __init__(
        ...
        train_nbunches = np.array(
            [np.sum(pattern[start_trains[ii] : end_trains[ii]]) for ii in range(len(start_trains))]
        )
        ...
        # Unused space
        needed_nslots = (
            (n_injections - 1) * min_MKI_slots + np.sum(list(map(len, inj_patterns))) + agap_length
        )
    ...
    def belongs_to_group(self, slots_within_injection=None, n_bunches_in_injection=None):
        mask = self.pattern > 0
        ...
        if slots_within_injection is not None:
            mask = np.logical_and(
                mask,
                np.array([(ss in slots_within_injection) for ss in self.slot_within_injection]),
            )
...
class FillingPattern(object):
    ...
    def from_json(cls, fname):
        with open(fname, "r") as fid:
            data = json.load(fid)
        patt = cls(
            pattern_b1=data["beam1"], pattern_b2=data["beam2"], scheme_name=fname.split(".json")[0]
        )
        return patt
    ...
    def from_csv(cls, fname):
        ...
        for bb in [1, 2]:
            ...
            filled_buckets = np.fromstring(
                patt_dict[f"B{bb} filled buckets"], sep=",", dtype=np.int64
            )
            ...
            # Get gaps in slots
            mki_gap_in_slots = int(
                np.round(float(patt_dict[f"B{bb} LHC injection kicker gap [ns]"]) / 25 + -1)
            )
            patt_dict[f"mki_gap_in_slots_b{bb}"] = mki_gap_in_slots
            mkp_gap_in_slots = int(
                np.round(float(patt_dict[f"B{bb} SPS injection kicker gap [ns]"]) / 25 + -1)
            )
            patt_dict[f"mkp_gap_in_slots_b{bb}"] = mkp_gap_in_slots
            abort_gap_in_slots = int(np.round(float(patt_dict[f"B{bb} abort gap [ns]"]) / 25 + -1))
        ...
        patt = cls(
            pattern_b1=patt_dict["pattern_array_b1"],
    ...
    def __init__(
        ...
        self.b1 = FillingPatternSingleBeam(
            pattern_b1, ring_length_slots, min_MKI_slots, min_MKP_slots, agap_first_slot
        )

        self.b2 = FillingPatternSingleBeam(
            pattern_b2, ring_length_slots, min_MKI_slots, min_MKP_slots, agap_first_slot
        )
        ...
        self.n_coll_ALICE = np.sum(self.b2.pattern * np.roll(self.b1.pattern, 891))
        self.n_coll_LHCb = np.sum(self.b1.pattern * np.roll(self.b2.pattern, 894))
    ...
    def to_csv(self, fname="auto"):
        ...
        txtlines.append(
            ",".join(["B1 injection buckets"] + ["%d" % (bb * 10 + 1) for bb in self.b1.inj_slots])
        )
        txtlines.append(
            ",".join(["B2 injection buckets"] + ["%d" % (bb * 10 + 1) for bb in self.b2.inj_slots])
        )
        txtlines.append(
            ",".join(["B1 injection n. bunches"] + ["%d" % bb for bb in self.b1.inj_nbun])
        )
        txtlines.append(
            ",".join(["B2 injection n. bunches"] + ["%d" % bb for bb in self.b2.inj_nbun])
        )
        ...
        txtlines.append(
            ",".join(
                ["B1 LHC injection kicker gap [ns]", "%d" % ((self.b1.actual_MKI_slots + 1) * 25)]
            )
        )
        txtlines.append(
            ",".join(
                ["B2 LHC injection kicker gap [ns]", "%d" % ((self.b2.actual_MKI_slots + 1) * 25)]
            )
        )
        txtlines.append(
            ",".join(
                ["B1 SPS injection kicker gap [ns]", "%d" % ((self.b1.actual_MKP_slots + 1) * 25)]
            )
        )
        txtlines.append(
            ",".join(
                ["B2 SPS injection kicker gap [ns]", "%d" % ((self.b2.actual_MKP_slots + 1) * 25)]
            )
        )
        txtlines.append(",".join(["B1 abort gap [ns]", "%d" % ((self.b1.agap_length + 1) * 25)]))
        txtlines.append(",".join(["B2 abort gap [ns]", "%d" % ((self.b2.agap_length + 1) * 25)]))
    ...
    def compute_beam_beam_schedule(self, n_lr_per_side):
        ...
        for bb in [self.b1, self.b2]:
            for ee in ["ATLAS/CMS", "ALICE", "LHCB"]:
                bb.bb_schedule[f"collides in {ee}"] = ~(
                    np.isnan(bb.bb_schedule[f"HO partner in {ee}"])
                )
