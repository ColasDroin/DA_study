...
if __name__ == "__main__":
    for bunch_nb in [
        ...
        root = tree_maker.tree_from_json(
            f"tree_maker_opt_flathv_75_1500_withBB_chroma5_1p4_{bunch_nb}.json"
        )
        ...
        if root.has_been("completed"):
            ...
            if all([node.has_been("completed") for node in root.generation(1)]):
                ...
                my_run_on.submit(my_file)
                if all([node.has_been("completed") for node in root.generation(2)]):
                    my_run_on = cluster(root.parameters["generations"]["3"]["run_on"])
                    my_file = "third_generation.sub"
                    my_run_on.create_sub_file(root.generation(3), "third_generation.sub")
                    my_run_on.submit(my_file)
            #          node.smart_run()
