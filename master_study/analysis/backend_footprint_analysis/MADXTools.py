...
import backend_footprint_analysis.WCTools as WCTools
...
colorDict.update(
    {"sbend": "C0", "rbend": "C0", "quadrupole": "C1", "rfcavity": "C3", "sextupole": "C2"}
)
...
def MADTrackParticles(coordinates, NTurns=1, saveFile=None, onepass="onepass"):
    ...
    madCall = (
        f"track,dump,{onepass}, onetable = true,file=trackone.trk;\n"
        "\n"
        f"!{40*'-'}\n"
        f"{trackingCmds};\n"
        f"!{40*'-'}\n"
...
def seqedit(sequence, editing, makeThin=False):
    ...
    def installStr(row):
        return (
            f'{row["mode"]},element = {row["name"]},class={row["definition"].split(":")[0]},at ='
            f' {row["at"]};'
        )
    ...
    elementsEntry = "\n".join(
        filter(None, [entryStr[row["mode"]](row) for _, row in editing.iterrows()])
    )
...
def plotSurvey(surveyTable):
    ...
    for gr in list(_groupedSurvey.groups.keys()):
        ...
        plt.plot(
            _groupedSurvey.get_group(gr)["z"],
            _groupedSurvey.get_group(gr)["x"],
            alpha=thisAlpha,
            linestyle="None",
            color=colorDict[gr],
            marker=markerDict[gr],
            label=gr,
        )
    ...
    for index, row in IP_loc.iterrows():
        plt.plot(
            ...
            ms=10,
            mew=1,
            marker=(1, 1, 90 + np.rad2deg(row["rotation"])),
            linestyle="None",
            alpha=1,
        )
        plt.text(
            ...
            ha="center",
            va="center",
            rotation=-90 + np.rad2deg(np.mod(row["rotation"], np.pi)),
        )
    ...
    for i in range(0, 8):
        ...
        plt.text(
            0.85 * axScale * thisArc.iloc[len(thisArc) // 2]["z"],
            0.85 * axScale * thisArc.iloc[len(thisArc) // 2]["x"],
            f"ARC{arc}",
            fontsize=7,
            ha="center",
            va="center",
            rotation=-90 + np.rad2deg(np.mod(thisArc.iloc[len(thisArc) // 2]["rotation"], np.pi)),
        )
...
def plotTwiss(twissTable):
    ...
    for gr in list(_groupedTwiss.groups.keys()):
        thisAlpha = 0.3 if colorDict[gr] == "k" else 1
        plt.plot(
            _groupedTwiss.get_group(gr)["s"],
            _groupedTwiss.get_group(gr)["betx"],
            alpha=thisAlpha,
            linestyle="None",
            color=colorDict[gr],
            marker=markerDict[gr],
