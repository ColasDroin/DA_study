...
class InteractionPoint:
    ...
    def make_bb_df(
        ...
        _BB.insert(
            10, "k1", [self.b2.strong_knl(_dx, _dy)[0][1] for _dx, _dy in zip(_BB["dx"], _BB["dy"])]
        )
        _BB.insert(
            11, "k3", [self.b2.strong_knl(_dx, _dy)[0][3] for _dx, _dy in zip(_BB["dx"], _BB["dy"])]
        )
...
class Beam:
    def __init__(
        self, beam, twiss, survey, Nb=None, E=None, emittx_n=None, emitty_n=None, dp_p0=None
    ):
        self.__type__ = "Beam"
        self.name = beam
        self.num = beam[-1]
    ...
    def strong_knl(self, dx, dy):
        ...
        n = np.arange(12 + 1)
        integratedComp = (
            -cst.mu0 * (IL_eq) * sciSpec.factorial(n) / (2 * np.pi) / (dx + 1j * dy) ** (n + 1)
        )
    ...
    def xi(self):
        if self.emittx_n != self.emitty_n:
            warnings.warn(
                "Beam.xi is only defined for round beams, taking the average... (use"
                " Beam.get_xi_x,Beam.get_xi_y instead)"
            )
    ...
    def get_xi_x(self, s):
        return (
            ...
            * cst.r_p
            * self.get_betx(s)
            / (2 * np.pi * self.gamma_r * self.get_sigx(s) * (self.get_sigx(s) + self.get_sigy(s)))
        )
    ...
    def get_xi_y(self, s):
        return (
            ...
            * cst.r_p
            * self.get_bety(s)
            / (2 * np.pi * self.gamma_r * self.get_sigy(s) * (self.get_sigx(s) + self.get_sigy(s)))
        )
