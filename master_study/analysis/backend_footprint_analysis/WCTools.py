...
import backend_footprint_analysis.Constants as cst
...
def getMultiCoeff(I, L, pos, nmax=1):
    ...
    n = np.arange(nmax + 1)
    integratedComp = (
        -cst.mu0 * (I * L) * sciSpec.factorial(n) / (2 * np.pi) / (pos[0] + 1j * pos[1]) ** (n + 1)
    )
...
class wire:
    def __init__(self, x, y, I, L, madClass=None):
        ...
        if madClass is not None:
            self.toMad = {
                "multipole": self.toMad_multipole,
                "bb": self.toMad_bb,
                "tobias": self.toMad_tobias,
            }[madClass.lower()]
    ...
    def getField(self, xObs, yObs, order=np.inf):
        ...
        if order != np.inf:
            ...
            for n in np.arange(1, order + 1):
                complexFields += (
                    (Bref / self.r0)
                    * (-np.exp(-1j * n * self.theta0))
                    * (xObs / self.r0 + 1j * yObs / self.r0) ** (n - 1)
                )
        else:
            ...
            complexFields = (
                Bref
                * (np.conjugate(pos_Obs) - np.conjugate(pos_wire))
                / np.absolute(pos_Obs - pos_wire) ** 2
            )
    ...
    def getKick(self, xObs, yObs, Energy, order=np.inf):
        ...
        if order != np.inf:
            ...
            for n, KNL_n, KSL_n in zip(np.arange(order + 1), KNL, KSL):
                complexKicks += (
                    -(KNL_n + 1j * KSL_n) * (xObs + 1j * yObs) ** (n) / sciSpec.factorial(n)
                )
    ...
    def plotWireLocation(self, scaling=1, ax=None):
        ...
        ax.plot(
            [scaling * self.x], [scaling * self.y], "o", color="C1", fillstyle="none", markersize=12
        )
        if self.I > 0:
    ...
    def toMad_multipole(self, at, Energy, name="wire_multipole", order=20, BBORBIT=False):
        ...
        def_1of2 = (
            f'class_{name}_1of2 : MULTIPOLE,KNL = {{0,{",".join(KNL[1:].astype(str))}}},KSL ='
            f' {{0,{",".join(KSL[1:].astype(str))}}};'
        )
        def_2of2 = f"class_{name}_2of2 : KICKER, L=0, HKICK={HKICK}, VKICK={VKICK}, TILT=real;"

        if BBORBIT:
            thisWire = pd.DataFrame(
                {
            ...
            thisWire = pd.DataFrame(
                {"mode": ["install"], "name": [f"{name}"], "at": [at], "definition": [def_1of2]}
            )
...
def plotVecField(
    ...
    if len(color) == 0:
        ...
        if norm == "log":
            plt.imshow(
                ...
                origin="lower",
                norm=LogNorm(vmin=colorLim[0], vmax=colorLim[1]),
            )
        else:
            plt.imshow(
                currentNorm,
    ...
    if mode == "quiver":
        ...
        plt.quiver(
            scaling * X[::density, ::density],
            scaling * Y[::density, ::density],
            vecX[::density, ::density] / currentNorm[::density, ::density],
            vecY[::density, ::density] / currentNorm[::density, ::density],
        )  # ,width = maxArrowSize *0.05*scaling*(np.max(X)-np.min(X)),color = arrowColor)
        ...
        plt.streamplot(
            ...
            linewidth=arrowSize,
            color=arrowColor,
            start_points=scaling * seed_points.T,
        )
